...
path_Protocol = "C:\\Seegene\\Protocol\\Allplex Respiratory Panel.prcl" # protocol path, 프로토콜마다 다름
light = "FAM,HEX,Cal Red 610,Quasar 670" # 프로토콜마다 다를 수 있음
...
class WorkList_db_class(metaclass=Singleton):
    ...
    def Input_plrn_data(self, date, protocol_name, smp_num, plate_type, cap_type, ctrl_seq, pcr_bcd):
        self.date = date
        ...
        cur = conn.cursor()
        cur.execute('''
                    insert into Info_plrn(Date, Protocol_Name, Smp_Num, Plate_Type, Cap_Type, Ctrl_Seq, PCR_bcd)
                    values(?, ?, ?, ?, ?, ?, ?)
                    '''
                    ,(self.date, self.protocol_name, self.smp_num, self.plate_type, self.cap_type, self.ctrl_seq, self.pcr_bcd))
        conn.commit()
    ...
    def Use_PCR(self, pcr_bcd, test_count):
        ...
        if self.pcr_bcd == "Hidden Barcode":
            ...
            conn = sqlite3.connect(db_con)
            cur = conn.cursor()
            cur.execute("update Info_PCR set Test_count = Test_count - (%s) where PCR_bcd = (%s)" % ("'" + self.test_count + "'", "'" + self.pcr_bcd + "'"))
            conn.commit()
            cur.close()
            conn.close()
    ...
    def make_plrn(self, id_plrn):
        ...
        for i in range(smp_num + 2):
            if i < smp_num:
                smp += str(plt_pos[i%8]) + str((i//8) + 1).zfill(2) + f",{light},,,Unknown,{smp_bcd[i]},,,,,,1_1_0,,,,,,,,,{Protocol_Name},,,,,," + "\n"
            elif i == smp_num:
                ctrl_1 = str(plt_pos[i%8]) + str((i//8) + 1).zfill(2)
            else:
                ctrl_2 = str(plt_pos[i%8]) + str((i//8) + 1).zfill(2)
        if ctrl_seq == "NC, PC":
            smp += f"{ctrl_1}" + f",{light},,,Negative Control,NC,,,,,,0_1_0,,,,,,,,,{Protocol_Name},,,,,," + "\n"
            smp += f"{ctrl_2}" + f",{light},,,Positive Control,PC,,,,,,0_1_0,,,,,,,,,{Protocol_Name},,,,,," + "\n"
        elif ctrl_seq == "PC, NC":
            smp += f"{ctrl_1}" + f",{light},,,Positive Control,PC,,,,,,0_1_0,,,,,,,,,{Protocol_Name},,,,,," + "\n"
            smp += f"{ctrl_2}" + f",{light},,,Negative Control,NC,,,,,,0_1_0,,,,,,,,,{Protocol_Name},,,,,," + "\n"
    ...
    Run Protocol,{path_Protocol},,Protocol File Name,,,,,,,,,,,,,,,,,,,,,,,,,,
    Data File,,,,,,,,,,,,,,,,,,,,,,,,,,,,,
    TBD,,,,,,,,,,,,,,,,,,,,,,,,,,,,,
    ...
    def Inst_bcd_path(self, bcd_path):
        ...
        name = cur.fetchall()
        if name == []:
            cur.execute("insert into Temp(Protocol_Name, Plate_Type, Cap_Type) values(?, ?, ?)", (protocol_name[-1], "Plate", "Cap"))
            conn.commit()
        cur.close()
        conn.close()
    ...
    def save_Temp(self, protocol, plate_type, cap_type):
        conn = sqlite3.connect(db_con)
        cur = conn.cursor()
        cur.execute("update Temp set (Plate_Type, Cap_Type) = ((%s), (%s)) where Protocol_Name = (%s)" % ("'" + plate_type + "'", "'" + cap_type + "'", "'" + protocol + "'"))
        conn.commit()
        cur.close()
        conn.close()
    ...
    def load_setting(self):
        ...
        cur.close()
        conn.close()
        return setting_data[0][1], setting_data[0][2], setting_data[0][3]
