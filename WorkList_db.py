...
class WorkList_db_class(metaclass=Singleton):
    ...
    def Input_plrn_data(self, date, protocol_name, smp_num, plate_type, cap_type, ctrl_seq, pcr_bcd, bcd_list):
        ...
        cur.execute('''
                    ...
                    values(?, ?, ?, ?, ?, ?, ?, ?)
                    '''
                    ,(self.date, self.protocol_name, self.smp_num, self.plate_type, self.cap_type, self.ctrl_seq, self.pcr_bcd, self.bcd_list))
        conn.commit()
    ...
    def make_plrn(self, id_plrn):
        ...
        for i in range(smp_num + 2):
            if i < smp_num:
                smp += str(plt_pos[i%8]) + str((i//8) + 1).zfill(2) + f",{info_protocol[0][1]},,,Unknown,{smp_bcd[i]},,,,,,1_1_0,,,,,,,,,{Protocol_Name},,,,,," + "\n"
            elif i == smp_num:
...
{smp}''')
    ...
    def Inst_bcd_path(self, bcd_path):
        ...
        name = cur.fetchall()
        if name == []:
            cur.execute("insert into Temp(Protocol_Name, Plate_Type, Cap_Type, Control) values(?, ?, ?, ?)", (protocol_name[-1], "Plate", "Cap", "NC, PC"))
            conn.commit()
        cur.close()
        conn.close()
    ...
    def save_Temp(self, protocol, plate_type, cap_type, ctrl_seq):
        conn = sqlite3.connect(db_con)
        cur = conn.cursor()
        cur.execute("update Temp set (Plate_Type, Cap_Type, Control) = ((%s), (%s), (%s)) where Protocol_Name = (%s)" % ("'" + plate_type + "'", "'" + cap_type + "'", "'" + ctrl_seq + "'", "'" + protocol + "'"))
        conn.commit()
        cur.close()
        conn.close()
